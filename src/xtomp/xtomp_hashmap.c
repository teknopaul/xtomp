...
static unsigned long crc32(ngx_str_t *s)
{
    unsigned int i;
    ...
    for ( i = 0 ;  i < s->len ;  i++ ) {
        char ch = s->data[i];
        crc32val = crc32_tab[(crc32val ^ ch) & 0xff] ^ (crc32val >> 8);
    }
    return crc32val;
}
 ...
 */
static unsigned int hashmap_hash_int(hashmap_t *map, ngx_str_t* keystring)
{
 ...
 */
static int hashmap_hash(hashmap_t *map, ngx_str_t* key)
{
    int curr;
    int i;

    /* If full, return immediately */
    if ( map->size >= (map->table_size/2) ) return MAP_FULL;
    ...
    for ( i = 0 ; i < MAX_CHAIN_LENGTH ; i++ ) {
        ...
        if ( map->data[curr].in_use == 1 && (xtomp_strcmp(map->data[curr].key, key) == 0) ) {
            return curr;
        }
 ...
 */
int hashmap_rehash(hashmap_t *map)
{
    int i;
    int old_size;
    ...
    hashmap_element* temp = (hashmap_element *) calloc(2 * map->table_size, sizeof(hashmap_element));
    if ( temp == NULL ) {
        return MAP_OMEM;
    }
    ...
    for ( i = 0 ; i < old_size ; i++ ) {
        ...
        status = hashmap_put(map, curr[i].key, curr[i].data);
        if (status != MAP_OK) {
            return status;
        }
    }
...
int hashmap_get(hashmap_t *map, ngx_str_t *key, any_t *arg)
{
    ...
    for( i = 0 ; i < MAX_CHAIN_LENGTH ; i++ ) {
        ...
        if ( in_use == 1 ) {
            if ( xtomp_strcmp(map->data[curr].key, key) == 0 ) {
...
int hashmap_iterate(hashmap_t *map, PFany f, any_t item) 
{
    ...
    if ( hashmap_length(map) <= 0 ) return MAP_MISSING;
